...
func TestPullRequestGenerator_Generate(t *testing.T) {
	...
	testCases := []struct {
		...
		// This test is broken because the fake driver returns not implemented
		// for label filtering.
		// {
...
func TestPullRequestGenerator_GetTemplate(t *testing.T) {
	...
	if diff := cmp.Diff(template, tpl); diff != "" {
		t.Fatalf("Template() failed:\n%s", diff)
	}
}

func TestPullRequestGenerator_AdditionalResources(t *testing.T) {
	template := &sourcev1.KustomizationSetTemplate{
		KustomizationSetTemplateMeta: sourcev1.KustomizationSetTemplateMeta{
			Labels: map[string]string{
				"cluster.app/name": "{{ cluster }}",
	...
	res, err := gen.AdditionalResources(sg)
	test.AssertNoError(t, err)

	want := []runtime.Object{}
	if diff := cmp.Diff(want, res); diff != "" {
		t.Fatalf("AdditionalResources() failed:\n%s", diff)
	}
}
